...
%*******reads data into************************
	...
	if filtip == 1
		fclose(fid);
   	if NS == 0 							
   		[x,Fs,Bits]=wavread(dosyadi);						%read all data of wav file
		else  
      	[x,Fs,Bits]=wavread(dosyadi,NS);					%read NS samples of wav file
   	end
	elseif filtip == 0
		if NS == 0 							
...
%		title('forward(—), reverse(...) flow signals'); 
      xlabel('Time(ms)');
      subplot(312); 
 %     imagesc(zfra,'Xdata',[tim_ax(1) tim_ax(NP)],'Ydata',[f1(1) f1(N)]/1000), axis tight, axis xy; 
      pcolor(tim_ax,f1/1000,zfra), axis tight, axis xy; shading interp; 
      ylabel('Frq(kHz)');  xlabel('Time(ms)');
		colormap(mycol); brighten(mycol,br);
...
%K=floor((1/dk)*log((NP*dt)/s0));
s=s0.*2.^([0:K-1]*dk);
s=[-(fliplr(s)) 0 s];
 
  % Wavelet transform:
[wave] = wav_bas(zp,s,time,dt,t0,K,m,mother);
w = wave((scl/16)+1:2*scl+(scl/16)+1,NP/2:3*NP/2-1);
...
%		meshc(tim_ax,s1,cs), axis tight;     
 ...
 %//////////////////this section is to apply some statistics to the FT  results/////////////////////////////
      ...
      figure(5);
		subplot(211);  plot(tim_ax,zf(1:NP),'r',tim_ax,zr(1:NP),'b'), axis tight;
 		subplot(212);
     	plot(tim_ax,zfrs_f(1:NP),'r',tim_ax,zfrs_r(1:NP),'b',tim_ax,css_f(1:NP),'m',tim_ax,css_r(1:NP),'c'), axis tight;
 ...
 %     imagesc(zfr,'Xdata',[tim_ax(1) tim_ax(NP)],'Ydata',[f1(1) f1(N)]/1000), axis tight, axis xy; 
   ...
   %   imagesc(cs,'Xdata',[tim_ax(1) tim_ax(NP)],'Ydata',[s1(1) s1(2*scl)]), axis tight, axis xy; 
      ...
      ylabel('Scale'); xlabel('Time(ms)'); 
		colormap(mycol); brighten(mycol,br);
      subplot(338);
      plot(tim_ax,css_f(1:NP),'r',tim_ax,css_r(1:NP),'b'), axis tight; xlabel('Time(ms)');
      subplot(339);
